...
ui <- navbarPage(
    ...
    tags$script(HTML("
      (function() {
        function resizePlotCard(card) {
          ...
          var maxHeight = parseInt(card.dataset.maxPlotHeight || '', 10);
          if (!isNaN(maxHeight) && maxHeight > 0) available = Math.min(available, maxHeight);
  ...
  tabPanel("History & Changes", dashboard_page(
    ...
    main = main_grid(
      ...
      grid_row(class = "history-absolute-row",
