...
source_data_paths <- function(project_dir = find_project_dir()) {
  # 部署目录把线上需要的 RDS 快照放在 data/ 里；shinyapps.io 只能访问 app 包内文件。
  # 如果 data/ 不存在，再回退到开发机相邻的 R_Union，方便本地诊断部署副本。
  app_data_dir <- file.path(project_dir, "data")
  deploy_paths <- list(
    wide_rates = file.path(app_data_dir, "WIDE_RATES"),
    zero_curve = file.path(app_data_dir, "ZERORATE_CURVE"),
    original_script = file.path(app_data_dir, "YieldCurve.R")
  )
  if (file.exists(deploy_paths$wide_rates) && file.exists(deploy_paths$zero_curve)) {
    return(deploy_paths)
  }
...
load_market_data <- function(project_dir = find_project_dir()) {
  ...
  required_paths <- paths[c("wide_rates", "zero_curve")]
  missing <- names(required_paths)[!file.exists(unlist(required_paths))]
