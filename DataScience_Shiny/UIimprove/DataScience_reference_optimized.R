# ============================================================
# DataScience 原始绘图参考脚本：可运行优化副本
# ============================================================
# 整体作用：保留原始分析、模型和绘图逻辑，仅修正 package 环境与 WIDE_* 数据路径。
# generate_gallery.R 会逐段运行本文件；某一步失败时记录错误并继续生成后面的图。

# Rblpapi 只用于连接 Bloomberg，本脚本绘图使用已经保存的 WIDE_* 数据，因此不需要连接终端。
# library(Rblpapi)
...
library(HH)
library(car)
# bruceR 和 RVAideMemoire 当前不在项目库中；对应检验失败时，图册生成器会记录并继续。
# library(bruceR)
...
# ggbiplot 不再提供稳定的 Windows CRAN 安装包；这里用 factoextra 生成统计含义相同的 PCA biplot。
ggbiplot <- function(pc, obs.scale = 1, var.scale = 1, groups = NULL,
                     ellipse = FALSE, circle = FALSE, ellipse.prob = 0.68) {
  factoextra::fviz_pca_biplot(
    pc,
    geom.ind = "point",
    habillage = groups,
    addEllipses = ellipse,
    ellipse.level = ellipse.prob,
    repel = TRUE
  )
}


# 从优化脚本所在位置找到 DataScience_Shiny，再从项目 data 文件夹读取复制后的 WIDE_*。
# 逐段运行时可能拿不到 source() 文件路径，因此回退到当前项目下的 UIimprove 文件。
reference_file <- tryCatch(
  normalizePath(sys.frames()[[1]]$ofile, winslash = "/", mustWork = TRUE),
  error = function(error) normalizePath(file.path(getwd(), "UIimprove", "DataScience_reference_optimized.R"), winslash = "/", mustWork = TRUE)
)
project_dir <- normalizePath(file.path(dirname(reference_file), ".."), winslash = "/", mustWork = TRUE)
data_dir <- file.path(project_dir, "data")

r=readRDS(file=file.path(data_dir, "WIDE_RATES"))
f=readRDS(file=file.path(data_dir, "WIDE_FX")) 
v=readRDS(file=file.path(data_dir, "WIDE_VOL"))
eco=readRDS(file=file.path(data_dir, "WIDE_ECO"))
c=readRDS(file=file.path(data_dir, "WIDE_CFTC"))
m=readRDS(file=file.path(data_dir, "WIDE_MM"))
eq=readRDS(file=file.path(data_dir, "WIDE_EQ"))
comm=readRDS(file=file.path(data_dir, "WIDE_COMM"))
cd=readRDS(file=file.path(data_dir, "WIDE_CREDIT"))
x=readRDS(file=file.path(data_dir, "WIDE_ALLX"))#better eco data
...
x <- as.data.table(readRDS(file=file.path(data_dir, "WIDE_ALLX")))
r <- as.data.table(r)
...
# robust 当前没有可用的 Windows package；MASS::epil 是同一个癫痫发作计数案例的逐期数据。
# aggregate() 将每位病人的四期发作次数加总，建立原代码需要的 sumY、Base、Age 和 Trt 字段。
breslow.dat <- aggregate(y ~ subject + base + age + trt, data = MASS::epil, FUN = sum)
names(breslow.dat)[names(breslow.dat) == "y"] <- "sumY"
names(breslow.dat)[names(breslow.dat) == "base"] <- "Base"
names(breslow.dat)[names(breslow.dat) == "age"] <- "Age"
names(breslow.dat)[names(breslow.dat) == "trt"] <- "Trt"
...
# ggbiplot 已不在当前 CRAN Windows binary 中；其他 PCA biplot 图仍会正常生成。
# library(ggbiplot)
